            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              <div className="bg-white rounded-xl border p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setHidden(!hidden)} aria-label={hidden ? 'Show balance' : 'Hide balance'} className="text-gray-400 hover:text-fedha-navy">
                    {hidden ? (
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17.94 17.94A10.94 10.94 0 0 1 12 20c-7 0-11-8-11-8a20.3 20.3 0 0 1 5.06-5.94M9.9 4.24A10.94 10.94 0 0 1 12 4c7 0 11 8 11 8a20.3 20.3 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><path d="M1 1l22 22"/></svg>
                    ) : (
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                    )}
                  </button>
                </div>
                <p className="text-2xl font-bold mt-1 text-fedha-navy">
                  {hidden ? '••••••' : formatMoney(summary.totalBalance, summary.currency)}
                </p>
              </div>
              <Card
            ...
            <div className="bg-white rounded-xl border shadow-sm divide-y">
              ...
              {summary.accounts.map((a) => (
                <div key={a.id} className="p-4 flex justify-between items-center">
                  ...
                  <p className="font-semibold">{hidden ? '••••••' : formatMoney(a.currentBalance, summary.currency)}</p>
                </div>
              ))}
            </div>
          </>
